...
function Gig() {
  ...
  useEffect(() => {
    const fetchGigData = async () => {
      try {
        setLoading(true);
        const response = await fetch(`http://localhost:3000/api/auth/gig/${id}`);
  ...
  if (!gigData) return <div className="flex justify-center items-center min-h-screen">No gig data found</div>;

  // Make sure we have an array of images to work with
  const images = (gigData.images && Array.isArray(gigData.images) && gigData.images.length > 0) 
    ? gigData.images 
    : defaultImages;
    ...
    <div className="bg-[#F7F9FC] min-h-screen">
      <div className="max-w-[1400px] mx-auto px-4 py-8">
        <div className="flex flex-col lg:flex-row gap-8">
          ...
          <div className="lg:w-2/3">
            <nav className="text-[#74767E] text-sm mb-5">
              {gigData.category || "Graphics & Design"} {">"} {gigData.subcategory || "Website Design"} {">"} {gigData.service || "Website Development"}
            </nav>

            <h1 className="text-[#404145] text-[28px] font-bold mb-6">
              {gigData.title || "Professional and responsive website design and development"}
            </h1>
            ...
            <div className="flex items-center gap-4 mb-6">
              <img
                className="w-12 h-12 rounded-full object-cover"
                src={gigData.freelancer_id?.full_verification?.[0]?.profile_pic || "https://images.unsplash.com/photo-1472099645785-5658abf4ff4e?w=100&h=100&fit=crop"}
                alt="Seller"
              />
              <div className="flex items-center gap-4">
                <span className="font-semibold text-[#404145]">
                  {gigData.freelancer_id?.full_verification?.[0]?.full_name || "John Developer"}
                </span>
                <div className="flex items-center gap-1">
                  <div className="flex">
                    {[...Array(5)].map((_, i) => (
                      <Star
                        key={i}
                        className={`w-4 h-4 ${i < Math.round(gigData.reviews?.rating || 5) ? "fill-[#FFB33E] text-[#FFB33E]" : "fill-[#E4E5E7] text-[#E4E5E7]"}`}
                      />
                    ))}
                  </div>
                  <span className="text-[#FFB33E] font-medium">{gigData.reviews?.rating || "5.0"}</span>
                  <span className="text-[#74767E]">({gigData.reviews?.count || "127"})</span>
                </div>
              </div>
            </div>
            ...
            <div className="mb-10 relative">
              ...
              <div style={{ height: "600px" }} className="relative overflow-hidden rounded-lg">
                <Slider {...settings}>
            ...
            {activeTab === "about" ? (
              <>
                ...
                <div className="bg-white p-8 rounded-lg shadow-sm mb-8">
                  ...
                  <div className="text-[#62646A] leading-7 space-y-4">
                    <p>{gigData.description || "I will create a professional and responsive website tailored to your needs, ensuring it looks great on all devices and provides an excellent user experience."}</p>
                ...
                <div className="bg-white p-8 rounded-lg shadow-sm">
                  ...
                  <div className="flex items-start gap-6">
                    <img
                      src={gigData.freelancer_id?.full_verification?.[0]?.profile_pic || "https://images.unsplash.com/photo-1472099645785-5658abf4ff4e?w=150&h=150&fit=crop"}
                      alt="Seller"
                      className="w-24 h-24 rounded-full object-cover"
                    />
                    <div className="space-y-3">
                      <h3 className="font-medium text-lg">{gigData.freelancer_id?.full_verification?.[0]?.full_name || "John Developer"}</h3>
                      <p className="text-[#62646A]">{gigData.portfolio.skills[0].name || "Full Stack Web Developer"}</p>
                      <div className="flex items-center gap-2">
                        <div className="flex">
                          {[...Array(5)].map((_, i) => (
                            <Star
              ...
              <div className="bg-white p-8 rounded-lg shadow-sm">
                <div className="flex items-center justify-between mb-8">
                  ...
                  <div className="flex items-center gap-2">
                    <span className="text-[#FFB33E] font-medium text-xl">
                      {gigData.reviews?.rating || "5.0"}
                    </span>
                    <div className="flex">
                      {[...Array(5)].map((_, i) => (
                        <Star
                          key={i}
                          className={`w-5 h-5 ${i < Math.round(gigData.reviews?.rating || 5) ? "fill-[#FFB33E] text-[#FFB33E]" : "fill-[#E4E5E7] text-[#E4E5E7]"}`}
                        />
                      ))}
                    </div>
                    <span className="text-[#74767E]">({gigData.reviews?.count || "127"})</span>
                ...
                <div className="space-y-8">
                  {(gigData.reviews?.items && gigData.reviews.items.length > 0) ? (
                    gigData.reviews.items.map((review) => (
                      <div
                        key={review.id}
                        className="border-b border-gray-100 pb-8"
                      >
                        <div className="flex items-start justify-between mb-4">
                          <div className="flex items-center gap-4">
                            <img
                              src={review.avatar || "https://images.unsplash.com/photo-1494790108377-be9c29b29330?w=100&h=100&fit=crop"}
                              alt={review.name}
                              className="w-12 h-12 rounded-full object-cover"
                            />
                            <div>
                        ...
                        <div className="flex mb-3">
                          {[...Array(5)].map((_, i) => (
                            <Star
                              key={i}
                              className={`w-4 h-4 ${i < review.rating ? "fill-[#FFB33E] text-[#FFB33E]" : "fill-[#E4E5E7] text-[#E4E5E7]"}`}
                            />
                          ))}
                        </div>
                    ...
                    [
                      {
                        ...
                        rating: 5,
                        date: "2 weeks ago",
                        review: "Amazing work! The website exceeded my expectations. The developer was very communicative and delivered everything on time. The design is beautiful and works perfectly on all devices. I'll definitely be coming back for more projects!",
                        helpful: 24,
                        notHelpful: 1,
                      },
                      {
                        ...
                        rating: 5,
                        date: "1 month ago",
                        review: "Exceptional service! The attention to detail was remarkable. They went above and beyond to ensure my website was exactly what I needed. The code is clean and well-documented. Highly recommend!",
                        helpful: 18,
                        notHelpful: 0,
                      }
                    ].map((review) => (
          ...
          <div className="lg:w-1/3">
            <div className="bg-white p-6 rounded-lg shadow-sm sticky top-6">
              <div className="flex justify-between items-center mb-6">
                <h3 className="text-lg font-medium">{gigData.packages?.standard?.title || "Standard Package"}</h3>
                <span className="text-2xl font-bold">${gigData.price || gigData.packages?.standard?.price || "99"}</span>
              </div>

              <p className="text-[#62646A] mb-6">
                {gigData.packages?.standard?.description || gigData.short_description || "Get a professional website with modern design, responsive layout, and optimized performance."}
              </p>

              <div className="flex justify-between mb-6 text-sm text-[#62646A]">
                <div className="flex items-center gap-2">
                  <Clock className="w-4 h-4" />
                  <span>{gigData.delivery_time || gigData.packages?.standard?.delivery_time || "4"} Days Delivery</span>
                </div>
                <div className="flex items-center gap-2">
                  <RefreshCw className="w-4 h-4" />
                  <span>{gigData.revision || gigData.packages?.standard?.revisions || "3"} Revisions</span>
                </div>
              </div>

              <div className="space-y-3 mb-6">
                {(gigData.features || gigData.packages?.standard?.features || [
                  "Responsive Design",
                  "Source Code",
                  "Content Upload",
                  "Design Customization",
              ...
              <button className="w-full bg-[#1DBF73] text-white py-3 rounded-md hover:bg-[#19a463] transition-colors font-medium">
                Continue (${gigData.price || gigData.packages?.standard?.price || "99"})
              </button>
